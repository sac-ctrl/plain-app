...
Android side:
- `helpers/CallRecorderHelper.kt` — singleton `MediaRecorder` wrapper. Source `MIC` (capturing the actual `VOICE_CALL` stream needs a system signature permission that user-installed apps cannot get; speakerphone is the documented workaround and the live-call page already prompts for it). Output: AAC inside MP4, mono 44.1 kHz, 96 kbps, with a sidecar `.json` carrying display name, source, direction, app id/name, timestamps, duration and size.
