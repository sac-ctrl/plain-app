<template>
  <v-modal @close="cancel">
    ...
    <template #content>
      <p class="sg-intro">Help us improve! Please answer one quick question to share your feedback.</p>
      <p class="sg-q">{{ disguise.currentQuestion }}</p>
      <p class="sg-hint">Your answer is private and stays on this device.</p>
      <div class="form-row">
        <v-text-field
          ref="aRef"
          v-model="answer"
      ...
      <div class="sg-tries" :class="{ warn: tries >= 2 }">
        Attempts remaining: {{ Math.max(0, 5 - tries) }}
      </div>
    </template>
    <template #actions>
...
onMounted(async () => {
  await nextTick()
  await disguise.ensureFirstTimeAnswerStored()
...
async function doAction() {
  err.value = ''
  if (!answer.value.trim()) { err.value = 'Please enter your answer'; return }
  loading.value = true
  try {
    const ok = await disguise.tryUnlock(answer.value)
    if (!ok) {
      tries.value += 1
      err.value = 'That answer does not match. Please try again.'
      if (tries.value >= 5) {
